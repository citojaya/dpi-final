        ...
        for(int j=jIndex-1; j<jIndex+2; j++){
            for(int i=iIndex-1; i<iIndex+2; i++){
                ...
                for(int n=0; n<bdBox[cI].totalFaces; n++){
                    ...
                    real jpX, jpY, jpZ;
                    // Rotor face
                    if(bdBox[cI].faceType[n] == 1){
                        jpX = wFaceRot[fId].centroid[0];
                        jpY = wFaceRot[fId].centroid[1];
                        jpZ = wFaceRot[fId].centroid[2];
                    }
                    // Stator face
                    else if(bdBox[cI].faceType[n] == 2){
                        jpX = wFace[fId].centroid[0];
                        jpY = wFace[fId].centroid[1];
                        jpZ = wFace[fId].centroid[2];
                    }
