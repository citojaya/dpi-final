...
void findContactFromMesh(Particle *p){
    ...
    if(nFace > 0){
        ...
        real gap;
        if(faceType == 1){
            getUnitVector(wFaceRot[nFace].node1,wFaceRot[nFace].node2,wFaceRot[nFace].node3,uVec);
            gap = getOverlap(demPart[p->part_id].pos,demPart[p->part_id].dia, wFaceRot[nFace].node1,
                                        wFaceRot[nFace].node2,wFaceRot[nFace].node3, uVec);
            //writeLogNum("wallmesh.log","GAP ",gap/lengthFactor);
       }
        else if(faceType == 2){
            getUnitVector(wFace[nFace].node1,wFace[nFace].node2,wFace[nFace].node3,uVec);
            gap = getOverlap(demPart[p->part_id].pos,demPart[p->part_id].dia, wFace[nFace].node1,
                                        wFace[nFace].node2,wFace[nFace].node3, uVec);           
        }
        if(gap < 0) //If contact exists calculate contact force
        {
